...
def configure(conf):
    ...
    if not conf.check_cfg(package='bluez', args=['--cflags', '--libs'], uselib_store='BLUETOOTH', mandatory=False):
        libbluetooth = conf.check_cc(lib='bluetooth',
                                  header_name='bluetooth/bluetooth.h',
                                  define_name='HAVE_BLUETOOTH',
                                  uselib_store='BLUETOOTH')
    else:
        conf.define ("HAVE_BLUETOOTH", 1)
    if not conf.get_define ("HAVE_BLUETOOTH"):
        conf.fatal ("Cannot find BLUETOOTH libraries")
...
def build (bld):
    ...
    PROGRAMS = 'ndnlink ndnld ndnldc ndnblue'
    for program in Utils.to_list (PROGRAMS):
        bld (target = program,
             features = "c cprogram",
             source = "tools/%s.c" % program,
             use = "NDNLP_COMMON CCNX SSL BLUETOOTH",
             includes = "src"
